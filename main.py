...
def check_variables(variables, values):
...
st.markdown(
    "<div><h1 style='text-align: center; color: #1EA9AE; font-family: Arial'> Equations Calculator</h1></div>",
    unsafe_allow_html=True)

st.markdown(
    "<div><h4 style='text-align: center; color: #00000; font-family: Arial'>Newtons Equation of linear motion </h4></div>",unsafe_allow_html=True)

st.markdown(
    "<div><b>The newtonian equations of linear motion using Displacement,Initial Velocity, Final Velocity, Acceleration And Time.</b></div>",unsafe_allow_html=True)


st.write(
    'Select any 3 known-valued variables to find out the other 2 variables equation and solution according to select your 3 variable valued:'
)

opts = [('s', 'Displacement'), ('u', 'Initial Velocity'),
        ('v', 'Final Velocity'), ('a', 'Acceleration'), ('t', 'Time')]

known_variables = {
    symbol: st.checkbox(f"{name} ({symbol})")
    for symbol, name in opts
}  #
...
if sum(known_variables.values()) < 3:
    ...
    st.write(
        'Enter their values in the same unit system. Accordingly, this calculator will return the values of the remaining 2 variables. '
    )
elif sum(known_variables.values()) > 3:
...
if check_variables(
    ...
    if st.button(
            'Click here to get the values of the rest two variables with equations'
    ) is True:
    ...
    u, v, a = ask('Initial Velocity (u)', 'Final Velocity (v)',
                  'Acceleration (a)')
    if st.button(
            'Click here to get the values of the rest two variables with equations'
    ) is True:
    ...
    if st.button(
        ...
        known_variables,
    ['v', 'a', 't']):  # ['Final Velocity (v)', 'Acceleration (a)', 'Time (t)']
    ...
    if st.button(
        ...
        known_variables,
    ['s', 'a', 't']):  # ['Displacement (s)', 'Acceleration (a)', 'Time (t)']
    s, a, t = ask('Displacement (s)', 'Acceleration (a)', 'Time (t)')
    if st.button(
            'Click here to get the values of the rest two variables with equations'
    ) is True:
        st.write(
            'The equations which are used to determine the values:\n' +
            '$$\n' +
        ...
        try:
            ...
            v = None
        st.write('Inserting your given values in these equations, we get: ')
        st.write('Initial Velocity $(u) = \\frac{s}{t} - \\frac{1}{2}at =$ ',
                 u)
        st.write('Final Velocity $(v) = \\frac{s}{t} + \\frac{1}{2}at =$ ', v)

elif check_variables(
        known_variables,
    ['s', 'v', 't']):  # ['Displacement (s)', 'Final Velocity (v)', 'Time (t)']
    ...
    if st.button(
        ...
        known_variables,
    ['s', 'v', 'a'
     ]):  # ['Displacement (s)', 'Final Velocity (v)', 'Acceleration (a)']
    ...
    if st.button(
        ...
        st.write('Time $(t)= \\frac{v}{a} - \\frac{\\sqrt{v^2 - 2as}}{a} =$ ',
                 t)

elif check_variables(
        known_variables,
    ['s', 'u', 't'
     ]):  # ['Displacement (s)', 'Initial Velocity (u)', 'Time (t)']
    ...
    if st.button(
        ...
        known_variables,
    ['s', 'u', 'a'
     ]):  # ['Displacement (s)', 'Initial Velocity (u)', 'Acceleration (a)']
    s, u, a = ask('Displacement (s)', 'Initial Velocity (u)',
                  'Acceleration (a)')
    if st.button(
            'Click here to get the values of the rest two variables with equations'
    ) is True:
        ...
        st.write(
            'Time $(t) = -\\frac{u}{a} + \\frac{\\sqrt{u^2 + 2as}}{a} =$ ', t)

elif check_variables(
        known_variables,
    ['s', 'u', 'v'
     ]):  # ['Displacement (s)', 'Initial Velocity (u)', 'Final Velocity (v)']
    s, u, v = ask('Displacement (s)', 'Initial Velocity (u)',
                  'Final Velocity (v)')
    if st.button(
            'Click here to get the values of the rest two variables with equations'
    ) is True:
